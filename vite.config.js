...
const curlFetch = (url, options = {}) => {
  return new Promise((resolve, reject) => {
    ...
    console.log(`[curlFetch] Executing: curl.exe ${method} to ${url}`);
...
// Custom Vite server plugin to proxy search and Yandex Music requests and bypass browser CORS
    ...
    server.middlewares.use(async (req, res, next) => {
      ...
      if (req.url.startsWith('/api/search')) {
        ...
        for (const inst of pipedInstances) {
          try {
            const response = await fetch(`${inst}/search?q=${encodeURIComponent(q)}&filter=videos`, {
              headers: {
      ...
      if (req.url.startsWith('/api/yandex')) {
        ...
        if (req.url.startsWith('/api/yandex/auth/device-code')) {
          try {
            const body = new URLSearchParams({
              client_id: '23cabbbdc6cd418abb4b39c32c41195d'
            });
            const response = await fetch('https://oauth.yandex.ru/device/code', {
              method: 'POST',
              headers: {
        ...
        if (req.url.startsWith('/api/yandex/auth/token')) {
        ...
        if (req.url.startsWith('/api/yandex/account')) {
          try {
            const response = await fetch('https://api.music.yandex.net/account/status', { headers });
            if (response.ok) {
              const data = await response.json();
              res.end(JSON.stringify(data));
            } else {
        ...
        // 2.2 Yandex Search
        if (req.url.startsWith('/api/yandex/search')) {
          const text = urlObj.searchParams.get('q') || urlObj.searchParams.get('query') || '';
          try {
            const response = await fetch(`https://api.music.yandex.net/search?text=${encodeURIComponent(text)}&type=all&page=0`, { headers });
            if (response.ok) {
              const data = await response.json();
              res.end(JSON.stringify(data));
            } else {
        ...
        if (req.url.startsWith('/api/yandex/likes/add') || req.url.startsWith('/api/yandex/likes/remove')) {
          ...
          const isAdd = req.url.startsWith('/api/yandex/likes/add');
          const endpoint = isAdd ? 'add-multiple' : 'remove';
          try {
            const response = await fetch(`https://api.music.yandex.net/users/${uid}/likes/tracks/${endpoint}`, {
              method: 'POST',
              headers: {
                ...headers,
                'Content-Type': 'application/x-www-form-urlencoded'
        ...
        if (req.url.startsWith('/api/yandex/likes')) {
          const uid = urlObj.searchParams.get('uid') || '';
          if (!uid) {
            res.statusCode = 400;
          ...
          try {
            const likesResponse = await fetch(`https://api.music.yandex.net/users/${uid}/likes/tracks`, { headers });
            if (!likesResponse.ok) {
        ...
        if (req.url.startsWith('/api/yandex/playlists')) {
          const uid = urlObj.searchParams.get('uid') || '';
          if (!uid) {
            res.statusCode = 400;
            res.end(JSON.stringify({ error: 'uid parameter is required' }));
          ...
          try {
            const response = await fetch(`https://api.music.yandex.net/users/${uid}/playlists/list`, { headers });
            if (response.ok) {
              const data = await response.json();
              res.end(JSON.stringify(data));
            } else {
        ...
        if (req.url.startsWith('/api/yandex/playlist-tracks')) {
          const uid = urlObj.searchParams.get('uid') || '';
          ...
          try {
            const response = await fetch(`https://api.music.yandex.net/users/${uid}/playlists/${kind}`, { headers });
            if (response.ok) {
              const data = await response.json();
        ...
        if (req.url.startsWith('/api/yandex/my-wave/info')) {
          try {
            const station = urlObj.searchParams.get('station') || 'user:onyourwave';
            const response = await fetch(`https://api.music.yandex.net/rotor/station/${encodeURIComponent(station)}/info`, { headers });
            if (response.ok) {
              const data = await response.json();
              res.end(JSON.stringify(data));
            } else {
        ...
        if (req.url.startsWith('/api/yandex/my-wave/settings')) {
          try {
            const station = urlObj.searchParams.get('station') || 'user:onyourwave';
            ...
            const response = await fetch(`https://api.music.yandex.net/rotor/station/${encodeURIComponent(station)}/settings3`, {
              method: 'POST',
              headers: {
                ...headers,
        ...
        if (req.url.startsWith('/api/yandex/my-wave')) {
          try {
            const station = urlObj.searchParams.get('station') || 'user:onyourwave';
            const response = await fetch(`https://api.music.yandex.net/rotor/station/${encodeURIComponent(station)}/tracks`, { headers });
            let tracks = [];
        ...
        if (req.url.startsWith('/api/yandex/radio-feedback')) {
          try {
            const stationId = urlObj.searchParams.get('stationId') || '';
            const trackId = urlObj.searchParams.get('trackId') || '';
            const event = urlObj.searchParams.get('event') || 'trackStarted'; // trackStarted, trackFinished, skip
            ...
            const response = await fetch(`https://api.music.yandex.net/rotor/station/${encodeURIComponent(stationId)}/feedback`, {
              method: 'POST',
              headers: {
                ...headers,
                'Content-Type': 'application/json'
        ...
        if (req.url.startsWith('/api/yandex/chart')) {
          try {
            const response = await fetch('https://api.music.yandex.net/landing3/chart', { headers });
            if (response.ok) {
              const data = await response.json();
        ...
        if (req.url.startsWith('/api/yandex/trends')) {
          ...
          try {
            const response = await fetch('https://api.music.yandex.net/landing3/new-releases', { headers });
            if (response.ok) {
              const data = await response.json();
              ...
              if (albumsToFetch.length > 0) {
                ...
                const fetchPromises = targetIds.map(async (id) => {
                  try {
                    const albRes = await fetch(`https://api.music.yandex.net/albums/${id}`, { headers });
                    if (albRes.ok) {
        ...
        if (req.url.startsWith('/api/yandex/artist-info')) {
          ...
          try {
            const response = await fetch(`https://api.music.yandex.net/artists/${encodeURIComponent(artistId)}/brief-info`, { headers });
            if (response.ok) {
              const data = await response.json();
              res.end(JSON.stringify(data));
            } else {
        ...
        if (req.url.startsWith('/api/yandex/album-tracks')) {
          ...
          try {
            const response = await fetch(`https://api.music.yandex.net/albums/${encodeURIComponent(albumId)}/with-tracks`, { headers });
            if (response.ok) {
              const data = await response.json();
              res.end(JSON.stringify(data));
            } else {
        ...
        if (req.url.startsWith('/api/yandex/lyrics')) {
          const trackId = urlObj.searchParams.get('trackId') || '';
          ...
          // Helper function for timeout-aborted fetch requests
          ...
          // Helper function for LRCLIB requests with curl fallback
          const fetchFromLrclib = async (url, timeoutMs = 2500) => {
            ...
            if (clientAbortController.signal.aborted) return null;
            // 2. Fallback to curl.exe (asynchronous)
            try {
              const timeoutSec = Math.max(1, Math.round(timeoutMs / 1000));
              const stdout = await new Promise((resolve, reject) => {
                const child = exec(`curl.exe -s -S -m ${timeoutSec} -H "User-Agent: MusicPlayer/1.0.0" "${url}"`, {
                  encoding: 'utf-8'
                }, (error, stdout, stderr) => {
                  if (error) {
                    reject(error);
          ...
          // Clean up track title by removing parenthesized and bracketed info and special chars
          ...
          try {
            // 1. Try Yandex Music API in parallel first (since it's very fast and official)
            if (trackId && trackId !== 'empty') {
              ...
              if (!isNaN(cleanTrackId)) {
                ...
                // Try to find synced lyrics first, then plain lyrics
            ...
            // 2. Fall back to LRCLIB and Lyrics.ovh in parallel ONLY if Yandex returned nothing
            if (!lyrics && title && artist) {
              ...
              // LRCLIB SEARCH (Fuzzy/Clean query)
              fallbackPromises.push((async () => {
                try {
              ...
              // Coordinator for fallback databases
              const finalResult = await new Promise((resolve) => {
                ...
                // Global maximum timeout for fallback lookup
                  ...
                  try {
                    ...
                    if (result && result.text) {
                      if (result.synced) {
                        ...
                        // Set short timeout since we already have plain lyrics
        ...
        if (req.url.startsWith('/api/yandex/stream')) {
          const trackId = urlObj.searchParams.get('trackId') || '';
          if (!trackId) {
            res.statusCode = 400;
          ...
          try {
            const infoRes = await fetch(`https://api.music.yandex.net/tracks/${trackId}/download-info`, { headers });
            if (!infoRes.ok) {
            ...
            const xmlRes = await fetch(downloadUrl, { headers });
            if (!xmlRes.ok) {
